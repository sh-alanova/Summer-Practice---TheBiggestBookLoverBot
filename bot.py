...
def introduction(message):
    ...
    m3 = (
        "▲ /recommendation <i>Genre</i> or <i>Random</i> --- return you some books of this genre or some random "
        "books\n "
    )
    genres = (
    ...
    ex4 = (
        "✻ <i>Example:</i> /image_of_author Pushkin or /image_of_author Aleksandr Pushkin or /image_of_author "
        "Пушкин\n\n "
    )
    ...
    if get_message_bot[:7] == "/author":
        ...
        if author:
            ...
            if send_mess is None:
                ...
                markup.add(
                    types.InlineKeyboardButton("More information", url=author_page.url)
                )
                bot.send_message(message.chat.id, send_mess, parse_mode="html")
                bot.send_message(
                    message.chat.id,
                    "But if you want, here you can read more about this author:",
                    parse_mode="html",
        ...
        if book:
            ...
            if send_mess is None:
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(
                    types.InlineKeyboardButton("More information", url=book_page.url)
                )
                bot.send_message(message.chat.id, send_mess, parse_mode="html")
                bot.send_message(
                    message.chat.id,
                    "But if you want, here you can read more about this book:",
                    parse_mode="html",
                    disable_web_page_preview=True,
                    reply_markup=markup,
                )
        ...
        if genre:
            ...
            if recommendation is None:
                send_mess = (
                    "Your query is uncorrect or you write the genre that I do not have. Please look again at "
